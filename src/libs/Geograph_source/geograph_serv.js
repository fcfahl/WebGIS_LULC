 ...
 */
	...
	function _get_textsearch(query,callback,location,per) {
                var url = "http://www.geograph.org.uk/syndicator.php?text="+encodeURIComponent(query)+"&format=JSON&callback=?";
	...
	function _get_tags(query,method,callback) {
		if (!method)
			method = 'tags/tags';
                ...
                var url = "http://www.geograph.org.uk/"+method+".json.php?q="+encodeURIComponent(query)+"&callback=?";
...
function markImage(image) {
	...
	if (current) {
		...
		if (current == image || current.search(re) > -1) {
			...
			newtext = 'Mark';
			$('a[href="http://'+geograph_domain+'/photo/'+image+'"] img').removeClass('marked');
		} else {
			newCookie = current + ',' + image;
			$('a[href="http://'+geograph_domain+'/photo/'+image+'"] img').addClass('marked');
		}
	} else {
	...
	i.src= "http://"+geograph_domain+"/stuff/record_mark.php?id="+id;
}
